...
import pygtk,gtk,glib,csv,string,sys,os,subprocess,re,time,struct,binascii,threading,pango,shutil,datetime
pygtk.require('2.0')
...
def getmysqlstring(plc):
	...
	mysqlstring = "%s, mysql_datetime TIMESTAMP DEFAULT CURRENT_TIMESTAMP" % mysqlstring
	return mysqlstring;
...
class TaskThread(threading.Thread):
	...
	def shutdown(self, widget, button2, button3,button,textbuffer,sw,button4):
		"""Stop this thread"""
		button3.hide()
		button2.show()
		button.show()
	...
	def run(self, widget, button2, button3, spinner, textbuffer, sw,button,button4,plc):
		button2.hide()
		button3.show()
		button.hide()
		button4.hide()
		self.setInterval(spinner.get_value())
		delay = spinner.get_value()
		globals()['counter' + plc] = 1
		count = int(self._interval*10)
		while not self._finished.isSet():
			if count == int(self._interval*10):
				self.task(textbuffer,delay,sw,button,button2,button3,button4,widget,plc)
				count = 1
			else:
	...
	def task(self,textbuffer,delay,sw,button,button2,button3,button4,widget,plc):
		...
		if client.connect() == False:
			...
			read_registers = []
			read_counter = 0
			read_start = globals()['StartAddress' + plc]

			# Read in 50 register chunks
			if globals()['NumOfRegisters' + plc] < 50:
				read_counter = globals()['NumOfRegisters' + plc]
			else:
				read_counter = 50

			while len(read_registers) < globals()['NumOfRegisters' + plc]:
				rr = client.read_holding_registers(read_start,read_counter)
				try:
					test = rr.registers[0]
				except:
				...
				read_registers = read_registers + rr.registers

				# next batch
				read_start = read_start+read_counter

				if globals()['NumOfRegisters' + plc] < read_start+50-1:
					read_counter = globals()['NumOfRegisters' + plc]-read_start+1
				else:
					read_counter = 50

			if read_registers[globals()['FlagReg' + plc]-1] != 0:
				...
				while (count < globals()['NumOfRegisters' + plc]):
					if globals()['DataTypes' + plc][count] != "0" and count != (globals()['FlagReg' + plc]-1):
						if globals()['DataTypes' + plc][count] == "1":
							temp = bin(read_registers[count])
							NewLineStr.append(temp.lstrip('0b'))
						elif globals()['DataTypes' + plc][count] == "2":
							NewLineStr.append("%o" % read_registers[count])
						elif globals()['DataTypes' + plc][count] == "3":
							NewLineStr.append("%0X" % read_registers[count])
						elif globals()['DataTypes' + plc][count] == "4":
							NewLineStr.append("%u" % read_registers[count])
						elif globals()['DataTypes' + plc][count] == "5":
							temp = "%i" % read_registers[count]
							if (int(temp) > 32767):
								temp = int(temp) - 65536
							NewLineStr.append(temp)
						elif globals()['DataTypes' + plc][count] == "6":
							temp = binascii.unhexlify("%0X" % read_registers[count])
							NewLineStr.append("%s" % temp)
						elif globals()['DataTypes' + plc][count] == "7":
							first = "%s" % ("%0X" % read_registers[count+1])
							second = "%s" % ("%0X" % read_registers[count])
							while (len(first) < 4):
								first = "0%s" % first
							while (len(second) < 4):
								second = "0%s" % second
							...
							first = "%s" % ("%0X" % read_registers[count])
							second = "%s" % ("%0X" % read_registers[count+1])
							while (len(first) < 4):
								first = "0%s" % first
							while (len(second) < 4):
								second = "0%s" % second
							...
							temp = bin(ReverseByteOrder(read_registers[count]))
							NewLineStr.append(temp.lstrip('0b'))
						elif globals()['DataTypes' + plc][count] == "12":
							NewLineStr.append("%o" % ReverseByteOrder(read_registers[count]))
						elif globals()['DataTypes' + plc][count] == "13":
							NewLineStr.append("%0X" % ReverseByteOrder(read_registers[count]))
						elif globals()['DataTypes' + plc][count] == "14":
							NewLineStr.append("%u" % ReverseByteOrder(read_registers[count]))
						elif globals()['DataTypes' + plc][count] == "15":
							temp = "%i" % ReverseByteOrder(read_registers[count])
							if (int(temp) > 32767):
								temp = int(temp) - 65536
							NewLineStr.append(temp)
						elif globals()['DataTypes' + plc][count] == "16":
							temp = binascii.unhexlify("%0X" % ReverseByteOrder(read_registers[count]))
							NewLineStr.append("%s" % temp)
						elif globals()['DataTypes' + plc][count] == "17":
							first = "%s" % ("%0X" % ReverseByteOrder(read_registers[count+1]))
							second = "%s" % ("%0X" % ReverseByteOrder(read_registers[count]))
							while (len(first) < 4):
								first = "0%s" % first
							while (len(second) < 4):
								second = "0%s" % second
							...
							first = "%s" % ("%0X" % ReverseByteOrder(read_registers[count]))
							second = "%s" % ("%0X" % ReverseByteOrder(read_registers[count+1]))
							while (len(first) < 4):
								first = "0%s" % first
							while (len(second) < 4):
								second = "0%s" % second
							...
							first = "%s" % bin(read_registers[count+1])
							first = first.lstrip('0b')
							second = "%s" % bin(read_registers[count])
							...
							first = "%s" % bin(read_registers[count])
							first = first.lstrip('0b')
							second = "%s" % bin(read_registers[count+1])
							second = second.lstrip('0b')
							while (len(first) < 16):
								first = "0%s" % first
							while (len(second) < 16):
							...
							first = "%s" % bin(ReverseByteOrder(read_registers[count+1]))
							first = first.lstrip('0b')
							second = "%s" % bin(ReverseByteOrder(read_registers[count]))
							second = second.lstrip('0b')
							while (len(first) < 16):
								first = "0%s" % first
							while (len(second) < 16):
							...
							first = "%s" % bin(ReverseByteOrder(read_registers[count]))
							first = first.lstrip('0b')
							second = "%s" % bin(ReverseByteOrder(read_registers[count+1]))
							second = second.lstrip('0b')
							while (len(first) < 16):
								first = "0%s" % first
							while (len(second) < 16):
				...
				fileOUT.writelines("%s" % RecordNo)
				fileOUT.close()
				rq = client.write_register(globals()['StartAddress' + plc]+globals()['FlagReg' + plc]-1, 0)
...
class TaskThreadMySQL(threading.Thread):
	"""Thread that executes a task every N seconds"""
		...
		if client.connect() == False:
			...
			if read_registers[globals()['FlagReg' + plc]-1] != 0:
				...
				if globals()['splitby' + plc] == "1":#Day of Week
					...
					# EndIF
					try:
						globals()['cur' + plc].execute('INSERT INTO %s%s VALUES(NULL,"%s",NULL)' % (globals()['mysqltablename' + plc] , dayofWeek[globals()['dayofWeek' + plc]],'","'.join(map(str, NewLineStr))))
					except :
					...
					# EndIF
					try:
						globals()['cur' + plc].execute('INSERT INTO %s%s VALUES(NULL,"%s",NULL)' % (globals()['mysqltablename' + plc] , monthofYear[globals()['month' + plc]],'","'.join(map(str, NewLineStr))))
					except :
						textbuffer.insert_at_cursor("[Database Error] Could not insert into MySQL database.\n")
						self.shutdown(widget, button2, button3,button,textbuffer,sw,button4)
						return
					...
					# EndIF
					try:
						globals()['cur' + plc].execute('INSERT INTO %s%s VALUES(NULL,"%s",NULL)' % (globals()['mysqltablename' + plc] , str(globals()['year' + plc]),'","'.join(map(str, NewLineStr))))
					except :
						textbuffer.insert_at_cursor("[Database Error] Could not insert into MySQL database.\n")
						self.shutdown(widget, button2, button3,button,textbuffer,sw,button4)
						return
				...
				###################################
					try:
						globals()['cur' + plc].execute('INSERT INTO %s VALUES(NULL,"%s",NULL)' % (globals()['mysqltablename' + plc],'","'.join(map(str, NewLineStr))))
					except :
						textbuffer.insert_at_cursor("[Database Error] Could not insert into MySQL database.\n")
						self.shutdown(widget, button2, button3,button,textbuffer,sw,button4)
						return
...
def startpoll(widget, button2, button3, spinner, textbuffer, sw,button,button4,plc):
	...
	globals()['polling' + plc].run(widget, button2, button3, spinner, textbuffer, sw,button,button4,plc)

def stoppoll(widget, button2, button3,button,textbuffer,sw,button4,plc):
	globals()['polling' + plc].shutdown(widget, button2, button3,button,textbuffer,sw,button4)

def startpollMySQL(widget, button2, button3, spinner, textbuffer, sw,button,button4,plc):
	globals()['polling' + plc] = TaskThreadMySQL()
	globals()['polling' + plc].run(widget, button2, button3, spinner, textbuffer, sw,button,button4,plc)

def stoppollMySQL(widget, button2, button3,button,textbuffer,sw,button4,plc):
	globals()['polling' + plc].shutdown(widget, button2, button3,button,textbuffer,sw,button4)
###############################################################
class TaskThreadC(threading.Thread):
	"""Thread that executes a task every N seconds"""
	def __init__(self):
		threading.Thread.__init__(self)
		self._finished = threading.Event()
	...
	def run(self, widget,delay,plc):
		...
		try:
			...
			print("\n");
			self.shutdown(widget,plc)
		sys.exit(0)

	def task(self,delay,widget,plc):
		"""The task done by this thread - override in subclasses"""
		logout = open("%s%slog.txt" % (textlog,time.strftime("%m%Y")), "a")
		fileIN = open("%s%s%s%sModBusDLData3" % (path,TheSlash,plc,TheSlash), "r")
		RecordNo = int(fileIN.readline())
...
class TaskThreadMySQLC(threading.Thread):
	"""Thread that executes a task every N seconds"""
...
def startpollC(widget, delay,plc):
	globals()['polling' + plc] = TaskThreadC()
	globals()['polling' + plc].run(widget, delay,plc)

def startpollMySQLC(widget, delay,plc):
	globals()['polling' + plc] = TaskThreadMySQLC()
	globals()['polling' + plc].run(widget, delay,plc)
	...
	def __init__(self,plc):
		...
		if globals()['databasetouse' + plc] == "1":
			...
			if globals()['splitby' + plc] == "1":#Day of Week
				...
				try:
					...
					try:
						...
						self.mainlabel = gtk.Label("Could not create CSV file.\nDid the folder name change?\nTry deleting the .ModBusDL folder in the root of your user directory and restart the program.")
						self.mainlabel.set_alignment(0.5, 0.5)
						self.mainlabel.set_line_wrap(True)
						self.main_vbox.pack_start(self.mainlabel, True, True, 0)

						self.button = gtk.Button("Close")
						self.button.connect("clicked", lambda w: gtk.main_quit())
						self.main_vbox.pack_start(self.button, False, True, 5)
			...
			try:
				...
				self.mainlabel = gtk.Label("Could not connect to MySQL database.\nError %d: %s" % (e.args[0],e.args[1]))
				self.mainlabel.set_alignment(0.5, 0.5)
				self.mainlabel.set_line_wrap(True)
				self.main_vbox.pack_start(self.mainlabel, True, True, 0)

				self.button = gtk.Button("Close")
				self.button.connect("clicked", self.restart,plc)
				self.main_vbox.pack_start(self.button, False, True, 5)
			...
			if globals()['splitby' + plc] == "1": #DayofWeek
				...
				if not globals()['cur' + plc].fetchall():
					...
					if globals()['createtable' + plc]:
						...
						self.mainlabel = gtk.Label("Could not find a table \"%s%s\" in the MySQL database.\n" % (globals()['mysqltablename' + plc],dayofWeek[globals()['dayofWeek' + plc]]))
						self.mainlabel.set_alignment(0.5, 0.5)
						self.mainlabel.set_line_wrap(True)
						self.main_vbox.pack_start(self.mainlabel, True, True, 0)
				...
				if not globals()['cur' + plc].fetchall():
					...
					if globals()['createtable' + plc]:
						...
						self.mainlabel = gtk.Label("Could not find a table \"%s\" in the MySQL database.\n" % globals()['mysqltablename' + plc])
						self.mainlabel.set_alignment(0.5, 0.5)
						self.mainlabel.set_line_wrap(True)
						self.main_vbox.pack_start(self.mainlabel, True, True, 0)
...
class MainProgC:
	...
	def __init__(self,delay,plc):
		...
		if not os.path.isfile("%s%s%s%sModBusDLData4" % (path,TheSlash,plc,TheSlash)):
			fileOUT = open("%s%s%s%sModBusDLData4" % (path,TheSlash,plc,TheSlash), "w")
			fileOUT.writelines("%s/%s/%s" % ((datetime.date.today() - datetime.timedelta( days = 1 )).day,datetime.date.today().month,datetime.date.today().year))
			fileOUT.close()
		fileIN = open("%s%s%s%sModBusDLData4" % (path,TheSlash,plc,TheSlash), "r")
		globals()['lastdate' + plc] = string.split(fileIN.readline().replace('\n', '').replace('\r', ''),'/')
		fileIN.close()
		if len(globals()['lastdate' + plc]) < 3:
			# There was an error during shutdown, set to today
			fileOUT = open("%s%s%s%sModBusDLData4" % (path,TheSlash,plc,TheSlash), "w")
			...
			fileOUT.close()
		###load in variables and update globals###
		fileIN = open("%s%s%s%sModBusDLData2" % (path,TheSlash,plc,TheSlash), "r")
		new_strings=string.split(fileIN.readline(),'|')
		globals()['StartAddress' + plc] = int(new_strings[0])
		globals()['NumOfRegisters' + plc] = int(new_strings[1])
		globals()['databasetouse' + plc] = new_strings[2]
		globals()['address' + plc] = new_strings[3]
		if globals()['databasetouse' + plc] == "1":
			globals()['LocOfCSV' + plc] = new_strings[4]
			globals()['CSVName' + plc] = new_strings[5]
			if new_strings[6] == "1":
				globals()['perdroptable' + plc] = True
			else:
				globals()['perdroptable' + plc] = False
			globals()['splitby' + plc] = new_strings[7]
			globals()['FlagReg' + plc] = int(new_strings[8])
			globals()['DataTypes' + plc] = string.split(new_strings[9].replace('\n', '').replace('\r', ''),',')
		else:
			globals()['mysqladdress' + plc] = new_strings[4]
			globals()['mysqlport' + plc] = new_strings[5]
			globals()['mysqlusername' + plc] = new_strings[6]
			globals()['mysqlpassword' + plc] = new_strings[7]
			globals()['mysqldbname' + plc] = new_strings[8]
			###
			if new_strings[9] == "1":
				globals()['perdroptable' + plc] = True
			else:
				globals()['perdroptable' + plc] = False
			globals()['splitby' + plc] = new_strings[10]
			globals()['mysqltablename' + plc] = ''.join(e for e in new_strings[11].replace(" ", "_") if e.isalnum())
			if new_strings[12] == "1":
				globals()['createtable' + plc] = True
			else:
				globals()['createtable' + plc] = False
			###
			globals()['FlagReg' + plc] = int(new_strings[13])
			globals()['DataTypes' + plc] = string.split(new_strings[14].replace('\n', '').replace('\r', ''),',')
		...
		if globals()['databasetouse' + plc] == "1":
			...
			try:
				...
				logout.writelines("%s - %s - Could not connect to MySQL database.\nError %d: %s\n" % (datetime.datetime.now(),plc,e.args[0],e.args[1]))
				return
			globals()['con' + plc].autocommit(True)
			globals()['cur' + plc] = globals()['con' + plc].cursor()
			...
			if globals()['splitby' + plc] == "1": #DayofWeek
				...
				if not globals()['cur' + plc].fetchall():
					...
					if globals()['createtable' + plc]:
						...
						#Throw Error
						print ("Could not find a table \"%s%s\" in the MySQL database.\n" % (globals()['mysqltablename' + plc] ,str(globals()['dayofWeek' + plc])))
						logout.writelines("%s - %s - Could not find a table \"%s%s\" in the MySQL database.\n" % (datetime.datetime.now(),plc,globals()['mysqltablename' + plc] ,str(globals()['dayofWeek' + plc])))
						return
			elif globals()['splitby' + plc] == "2": # Month
				## Check for startscript delete or (dates differ & persistant delete)
				if 'droptable' + plc in globals():
				...
				if not globals()['cur' + plc].fetchall():
					...
					if globals()['createtable' + plc]:
						...
						#Throw Error
						print ("Could not find a table \"%s%s\" in the MySQL database.\n" % (globals()['mysqltablename' + plc] ,str(globals()['month' + plc])))
						logout.writelines("%s - %s - Could not find a table \"%s%s\" in the MySQL database.\n" % (datetime.datetime.now(),plc,globals()['mysqltablename' + plc] ,str(globals()['month' + plc])))
						return
			elif globals()['splitby' + plc] == "3": # Year
				## Check for startscript delete or (dates differ & persistant delete)
				if 'droptable' + plc in globals():
				...
				if not globals()['cur' + plc].fetchall():
					...
					if globals()['createtable' + plc]:
						...
						globals()['cur' + plc].execute("create table %s(%s)" % (globals()['mysqltablename' + plc],mysqlstring))
					else:
						#Throw Error
						print ("Could not find a table \"%s\" in the MySQL database.\n" % globals()['mysqltablename' + plc])
						logout.writelines("%s - %s - Could not find a table \"%s\" in the MySQL database.\n" % (datetime.datetime.now(),plc,globals()['mysqltablename' + plc]))
						return
			#EndIF
			globals()['con' + plc].close()
		...
		if globals()['databasetouse' + plc] == "1":
			startpollC("enter", delay,plc)

		else:
			startpollMySQLC("enter", delay,plc)
...
def converter(self, data=None):
	...
	octalnum.connect("activate", self.enter_callback, octalnum)
	octalnum.set_text("")
	...
	hexnum.connect("activate", self.enter_callback, hexnum)
	hexnum.set_text("")
	...
	decnum.connect("activate", self.enter_callback, decnum)
	decnum.set_text("")
	...
	if globals()['NumOfRegisters' + plc] < 3:
		...
		if is_valid_ip(globals()['address' + plc]) and ((globals()['databasetouse' + plc] == "1") or (globals()['databasetouse' + plc] == "2" and is_valid_ip(globals()['mysqladdress' + plc]))):
			...
			if globals()['databasetouse' + plc] == "1":
				...
				tablename.connect("activate", self.enter_callback, tablename)
			...
			for count in range(0,globals()['NumOfRegisters' + plc]):
				...
				globals()['entry' + str(count)].connect("activate", self.enter_callback, globals()['entry' + str(count)])
				# byte/word ordering
				...
				globals()['entryAdd' + str(count)].connect("activate", self.enter_callback, globals()['entryAdd' + str(count)])
...
def __init__(self,plc):
	...
	entryip.connect("activate", self.enter_callback, entryip)
	...
	entryname.connect("activate", self.enter_callback, entryname)
	...
	entryLOC.connect("activate", self.enter_callback, entryLOC)
	...
	mysqlserver.connect("activate", self.enter_callback, mysqlserver)
	...
	mysqluser.connect("activate", self.enter_callback, mysqluser)
	...
	mysqlpass.connect("activate", self.enter_callback, mysqlpass)
	...
	mysqldb.connect("activate", self.enter_callback, mysqldb)
...
class startupwindow():
	...
	def newplc(self, widget):
		...
		newplcname.connect("activate", self.enter_callback, newplcname)
...
def main():
	...
	textlog = os.path.expanduser(os.path.join("~",".ModBusDL")) + TheSlash
